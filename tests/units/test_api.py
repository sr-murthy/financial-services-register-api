...
# -- Internal libraries --
import fs_register_api_client.api

from fs_register_api_client.api import FSRegisterApiSession, FSRegisterApiClient
from fs_register_api_client.constants import FS_REGISTER_API_CONSTANTS
from fs_register_api_client.exceptions import FSRegisterApiRequestException, FSRegisterApiResponseException


class _TestFSRegisterApi:

    _api_username = os.environ['API_USERNAME']
    _api_key = os.environ['API_KEY']


class TestFSRegisterApiSession(_TestFSRegisterApi):

    def test_fs_register_api_session(self):
        test_session = FSRegisterApiSession(self._api_username, self._api_key)
...
class TestFSRegisterApiClient(_TestFSRegisterApi):

    def test_fs_register_api_client____init__(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
        ...
        assert test_client.api_version == FS_REGISTER_API_CONSTANTS.API_VERSION.value

    def test_fs_register_api_client__common_search__api_request_exception_raised(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)

        with mock.patch('fs_register_api_client.api.FSRegisterApiSession.get') as mock_api_session_get:
            mock_api_session_get.side_effect = requests.RequestException('test RequestException')

            with pytest.raises(FSRegisterApiRequestException):
                test_client.common_search('exceptional resource', 'firm')

    def test_fs_register_api_client__common_search__no_api_request_exception(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)

        recv_response = test_client.common_search('hastings direct', 'firm')
        assert recv_response.ok
        assert recv_response.fs_register_data
        assert len(recv_response.fs_register_data)
        assert recv_response.fs_register_status == 'FSR-API-04-01-00'
        assert recv_response.fs_register_message == 'Ok. Search successful'
        assert recv_response.fs_register_resultinfo

        recv_response = test_client.common_search('non existent firm', 'firm')
        assert recv_response.ok
        assert not recv_response.fs_register_data
        assert recv_response.fs_register_status == 'FSR-API-04-01-11'
        assert recv_response.fs_register_message == 'No search result found'
        assert not recv_response.fs_register_resultinfo
    ...
    def test_fs_register_api_client___search_ref_number__resource_type_incorrect__value_error_raised(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)

        with pytest.raises(ValueError):
            test_client._search_ref_number('resource_name', 'incorrect_resource_type')

    def test_fs_register_api_client___search_ref_number__exceptional_request__api_request_exception_raised(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client___search_ref_number__response_not_ok__api_response_exception_raised(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)

        with mock.patch('fs_register_api_client.api.FSRegisterApiClient.common_search', return_value=mock.MagicMock(ok=False)):
            with pytest.raises(FSRegisterApiResponseException):
                test_client._search_ref_number('exceptional search', 'firm')
                test_client._search_ref_number('exceptional search', 'individual')
                test_client._search_ref_number('exceptional search', 'fund')

    def test_fs_register_api_client___search_ref_number__no_fs_register_data_in_response__api_response_exception_raised(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client___search_ref_number__fs_register_data_with_index_error__api_response_exception_raised(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client___search_ref_number__fs_register_data_with_key_error__api_response_exception_raised(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client___search_ref_number__incorrectly_specified_resource__no_fs_register_data__api_response_exception_raised(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client___search_ref_number__inadequately_specified_resource__nonunique_fs_register_data__api_response_exception_raised(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client___search_ref_number__correctly_and_adequately_specced_resource__unique_fs_register_data__response_returned_ok(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client___search_frn__correctly_and_adequately_specced_firm__unique_fs_register_data__response_returned_ok(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client___get_resource_info__invalid_resource_type__no_modifiers__value_error_raised(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)

        with pytest.raises(ValueError):
            test_client._get_resource_info('test_ref_number', 'invalid resource type')

    def test_fs_register_api_client___get_resource_info__invalid_resource_type__modifiers__value_error_raised(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)

        with pytest.raises(ValueError):
            test_client._get_resource_info('test_ref_number', 'invalid resource type', modifiers=('test_modifier1', 'test_modifier2',))

    def test_fs_register_api_client___get_resource_info__no_modifiers__request_exception_raised(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client___get_resource_info__modifiers__request_exception_raised(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client___get_resource_info__firm(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
        ...
        assert recv_response.ok
        assert recv_response.fs_register_data
        assert recv_response.fs_register_data[0]['Organisation Name'] == 'Hiscox Insurance Company Limited'
        ...
        assert recv_response.ok
        assert (
            recv_response.fs_register_data['PreviousAppointedRepresentatives'] or 
            recv_response.fs_register_data['CurrentAppointedRepresentatives']
        )
        ...
        assert recv_response.ok
        assert not recv_response.fs_register_data['PreviousAppointedRepresentatives']
        assert not recv_response.fs_register_data['CurrentAppointedRepresentatives']

    def test_fs_register_api_client___get_resource_info__fund(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client___get_resource_info__individual(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
        ...
        assert recv_response.ok
        assert recv_response.fs_register_data
        assert recv_response.fs_register_data[0]['Details']['Full Name'] == 'Mark Carney'
    ...
    def test_fs_register_api_client__get_firm(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_firm_names(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
        ...
        assert recv_response.ok
        assert recv_response.fs_register_data
        assert recv_response.fs_register_data[0]['Current Names'][0]['Name'] == 'Hiscox'
        assert recv_response.fs_register_data[1]['Previous Names']
    ...
    def test_fs_register_api_client__get_firm_addresses(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_firm_controlled_functions(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_firm_individuals(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_firm_permissions(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_firm_requirements(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_firm_requirement_investment_types(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_firm_regulators(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_firm_passports(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_firm_passport_permissions(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_firm_waivers(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_firm_exclusions(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_firm_disciplinary_history(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_firm_appointed_representatives(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
        ...
        assert recv_response.fs_register_data
        assert any([
            recv_response.fs_register_data['PreviousAppointedRepresentatives'],
            recv_response.fs_register_data['CurrentAppointedRepresentatives']
        ])
    ...
    def test_fs_register_api_client___search_irn__correctly_and_adequately_specced_individual__unique_fs_register_data__response_returned_ok(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_individual(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_individual_controlled_functions(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_individual_disciplinary_history(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client___search_prn__correctly_and_adequately_specced_fund__unique_fs_register_data__response_returned_ok(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_fund(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_fund_names(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_fund_subfunds(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
    ...
    def test_fs_register_api_client__get_regulated_markets(self):
        test_client = FSRegisterApiClient(self._api_username, self._api_key)
